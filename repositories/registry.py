...
class RepositoryRegistry:
    ...
    def get_repository(cls, name: str) -> Optional[BaseRepository]:
        """Ottiene un repository per nome"""
        cls.initialize()
    ...
    def refresh_repository(cls, name: str):
        ...
        if name in cls._repositories:
            try:
                cls._repositories[name].clear_cache()
                print(f"OpenShelf: Refreshed repository '{name}'")
            except Exception as e:
    ...
    def test_repository_connection(cls, name: str) -> Dict[str, Any]:
        ...
        if name not in cls._repositories:
            return {"status": "error", "message": f"Repository '{name}' not found"}
        
        repo = cls._repositories[name]
        ...
        try:
            ...
            if test_assets:
                ...
                return {
                    "status": "warning",
                    "message": f"Connected to '{name}' but no assets found"
                }
    ...
    def get_repository_info(cls, name: str) -> Dict[str, Any]:
        """Ottiene informazioni dettagliate su un repository"""
        cls.initialize()
